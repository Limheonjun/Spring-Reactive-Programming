package com.emgc.springreactiveprogramming.controller;
...
import com.emgc.springreactiveprogramming.service.ReactiveMathService;
...
public class ReactiveMathValidationController {
	...
	public Mono<Response> findSquare(@PathVariable int input){
		if(input < 10 || input > 20)
			throw  new InputValidationnException(input);
		...
		return Mono.just(input)
					.handle((integer, sink) -> {
						if(integer >= 10 && integer <= 20)
							...
							sink.error(new InputValidationnException(integer));
					})
			.cast(Integer.class)
